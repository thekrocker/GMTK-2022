...
public class ObjectPooling <T>: IPool<T> where T : MonoBehaviour, IPoolable<T>
{
    ...
    public ObjectPooling(GameObject pooledObject, int numToSpawn = 0) // For spawning at start..
    {
        prefab = pooledObject;
        Spawn(numToSpawn);
    ...
    public ObjectPooling(GameObject pooledObject, Action<T> pullObject, Action<T> pushObject, int numToSpawn = 0)
    {
        prefab = pooledObject;
